...
import com.cadastro.api.responses.Response;
...
public class ClienteController {
    ...
    @GetMapping(path = "/{id}")
    public ResponseEntity<Response<Cliente>> listarPorId (@PathVariable(name = "id") String id) throws NotFound {
        final Optional<Cliente> cliente = this.clienteService.listarPorId(id);

        if (!cliente.isPresent()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }

        return ResponseEntity.ok(new Response<Cliente>(cliente.orElseThrow(NotFound::new)));
    }
